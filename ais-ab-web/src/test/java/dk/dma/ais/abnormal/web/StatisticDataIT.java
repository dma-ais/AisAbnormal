...
public class StatisticDataIT {
    ...
    @Test
    public void testCanZoomInToSelectCellAndDisplayStatisticData() throws InterruptedException {
        browser.get("http://127.0.0.1:8080/abnormal");
        Thread.sleep(1000);
        IntegrationTestHelper.takeScreenshot(browser, "init");

        try {
            checkStatisticsMetadata();
            checkCellsAreDisplayedWhenZoomingIn();
            clickOnACell();
            checkStatisticDataDisplayedCorrectlyForClickedCell();
    ...
    private void checkStatisticsMetadata() {
